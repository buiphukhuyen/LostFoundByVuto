      ...
      if (cr_data.success === true) {
        ...
        cr_data.forEach(function (e) {
          if (e.gender1 === "FEMALE") {
            e.gender1 = "F";
            women++;
          } else if (e.gender1 === "MALE") {
            e.gender1 = "N";
            men++;
          } else if (e.gender1 === "LGBT") {
            e.gender1 = "G";
            lgbt++;
          } else {
            e.gender1 = "0";
          ...
          if (e.gender2 === "FEMALE") {
            e.gender2 = "F";
            women++;
          } else if (e.gender2 === "MALE") {
            e.gender2 = "N";
            men++;
          } else if (e.gender2 === "LGBT") {
            e.gender2 = "G";
            lgbt++;
          } else {
            e.gender2 = "0";
          ...
          cr += `(${e.gender1}) <a onclick="uinfo(${e.id1})">${e.id1}  </a>  - (${e.gender2})
                <a onclick="uinfo(${e.id2})">${e.id2}  </a>  - ${time}<br>`;
        });

        cr = `<br><b>PHÒNG CHAT (${cr_data.length} Cặp - ${
          cr_data.length * 2
        } người: 
              ${men} Nam, ${women} Nữ, ${lgbt} LGBT, ${unk} Ngẫu nhiên):</b><br>${cr}`;
        $("#ptnkcr").html(cr);
      } else {
  ...
  makeRequest(
    ...
    function (wr_data) {
      ...
      if (wr_data.success === true) {
        ...
        wr = `<br><b>PHÒNG CHỜ (${wr_data.length} người: ${men} Nam, ${women} Nữ, ${lgbt} LGBT, ${unk} Ngẫu nhiên):</b><br>${wr}`;
        $("#ptnkwr").html(wr);
      } else {
    ...
    function (data) {
      if (data.error === true) {
        if (data.errortype === "auth") {
          ...
          $("#ptnkinfo")
            .html(`<b>ID: ${id}</b><br>Couldn't get info for user ${id}<br>
      ...
      $("#ptnkinfo").html(`<b>ID: ${id}</b><br>${data.name} (<i>${
